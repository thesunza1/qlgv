...
public function add_CV_BC_HangNgay(Request $request)
{
    ...
    $nguoiDung = auth()->user();
    // Lấy giá trị lớn nhất của trường "BCHN_ID" từ bảng "BAOCAOHANGNGAY"
    $maxId = BaoCaoHangNgay::max('bchn_id');

    // Tăng giá trị lớn nhất lên 1 để có giá trị mới cho trường "BCHN_ID"
    $newId = $maxId + 1;
    ...
    $baoCao->bchn_tiendo = $request->input('bdhn_tiendo');
    $baoCao->bchn_noidung = $request->input('bchn_noidung');
    $baoCao->so_gio_lam = $request->input('so_gio_lam');
    $baoCao->lcv_id = $request->input('lcv_id');
    $baoCao->cv_id = $request->input('cv_id');
    $baoCao->bchn_id = $newId;
